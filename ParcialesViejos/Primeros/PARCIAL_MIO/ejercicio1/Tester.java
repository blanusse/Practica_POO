...
// - se paga interés por el TOTAL del saldo
        Account1 acc1 = new Account1(MonthDay.of(Month.JANUARY, 15));
// Se deposita un millón $1000000 en la cuenta remunerada
        acc1.depositFunds(1000000, MonthDay.of(Month.JANUARY, 18));
        acc1.depositFunds(50000, MonthDay.of(Month.JANUARY, 16));
...
// Se realiza una extracción de $500000
        acc1.withdrawFunds(500000, MonthDay.of(Month.JANUARY, 20));
        System.out.printf("$%.2f\n", acc1.getBalance()); // $552591,17
        acc1.withdrawFunds(100000, MonthDay.of(Month.JANUARY, 15));
...
// - se paga interés por el TOTAL del saldo
        Account1 acc2 = new Account2(MonthDay.of(Month.FEBRUARY, 15));
        acc2.depositFunds(1000000, MonthDay.of(Month.FEBRUARY, 18));
        acc2.depositFunds(50000, MonthDay.of(Month.FEBRUARY, 16));
        System.out.printf("$%.2f\n", acc2.getBalance()); // $1050000,00
        acc2.interestPayment(3);
        System.out.printf("$%.2f\n", acc2.getBalance()); // $1052591,17
        acc2.withdrawFunds(500000, MonthDay.of(Month.FEBRUARY, 20));
        System.out.printf("$%.2f\n", acc2.getBalance()); // $552591,17
        try {
// Falla porque la fecha de extracción debe ser posterior al 20/02
            acc2.withdrawFunds(100000, MonthDay.of(Month.FEBRUARY, 15));
        } catch (Exception ex) {
            System.out.println("Error"); // Error
        }
        System.out.printf("$%.2f\n", acc2.getBalance()); // $552591,17
        acc2.withdrawFunds(100000, MonthDay.of(Month.FEBRUARY, 21));
...
// - se paga interés SOLO HASTA 1 millón $1000000,00 del saldo
        Account1 acc3 = new Account3(MonthDay.of(Month.MARCH, 15));
        acc3.depositFunds(1000000, MonthDay.of(Month.MARCH, 18));
        try {
// Falla porque el monto de depósito debe ser mayor o igual a cien mil 100000
            acc3.depositFunds(50000, MonthDay.of(Month.MARCH, 16));
        } catch (Exception ex) {
            System.out.println("Error"); // Error
        }
        System.out.printf("$%.2f\n", acc3.getBalance()); // $1000000,00
        acc3.depositFunds(500000, MonthDay.of(Month.MARCH, 16));
...
// Pago Día 3: $1000000,00 * 0,3 * (1,0 / 365) = $821,91
        ...
        acc3.withdrawFunds(500000, MonthDay.of(Month.MARCH, 20));
        System.out.printf("$%.2f\n", acc3.getBalance()); // $1002465,75
        acc3.withdrawFunds(100000, MonthDay.of(Month.MARCH, 21));
